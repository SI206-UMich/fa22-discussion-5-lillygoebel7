...
class TestAllMethods(unittest.TestCase):
	...
	def setUp(self):
		...
		self.myWarehouse = Warehouse()
		#self.all_items = Warehouse([self.item1, self.item2, self.item3, self.item4, self.item5])

	## Check to see whether count_a works
	def test_count_a(self):
		self.assertEqual(count_a('all around'), 2, 'Tested count_a(all around)')
	...
	def test_add_item(self):
		...
		#self.myWarehouse.add_item(self.item5)
		#self.assertEqual(self.myWarehouse.items, [self.item1, self.item2, self.item3, self.item4, self.item5], 'Tested add_item(item5)')
		pass
